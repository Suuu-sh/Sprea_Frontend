  ...
  return <Section title="保有・取引履歴" description="終了すると拘束資金が解放されます。">
    <div className="card-list">
      {trades.map(trade => <article className="trade-card" key={trade.id}>
        <div><span className={`status-pill ${trade.status === "open" ? "active" : "ready"}`}>{trade.status === "open" ? "保有中" : "終了"}</span><h3><Link href={`/products/?key=${encodeURIComponent(trade.canonicalKey)}`}>{trade.title}</Link></h3><p>{trade.purchaseSource} → {trade.buybackSource}</p></div>
