...
export default function Home(){
 ...
 <section className="decision-observability" aria-label="本日の判定分析"><div className="observability-title"><div><b>本日の判定分析（JST）</b><small>BUYが出ない理由と、判定閾値までの差を表示します。</small></div><span>平均利益差 {yen(observability?.averageProfitGap??0)}</span></div><div className="observability-grid"><div className="decision-counts"><article><small>Opportunity</small><b>{observability?.today.opportunities??0}</b></article><article><small>BUY</small><b className="profit">{observability?.today.buys??0}</b></article><article><small>SKIP</small><b>{observability?.today.skips??0}</b></article></div><div className="reason-list"><b>SKIP理由</b>{(observability?.skipReasons??[]).map(item=><div key={item.reason}><span>{reasonLabels[item.reason]??item.reason}</span><strong>{item.count}件</strong></div>)}{!observability?.skipReasons.length&&<small>本日のSKIPはまだありません。</small>}<small>1件のOpportunityに複数理由が含まれる場合があります。</small></div></div>{Boolean(observability?.recentSkips.length)&&<div className="skip-gap-list"><b>直近のSKIPとBUYまでの差</b>{observability?.recentSkips.slice(0,8).map(item=><article key={item.id}><span><strong>{item.title}</strong><small>{item.reasons.map(reason=>reasonLabels[reason]??reason).join("・")}</small></span><span><small>利益</small><b>{yen(item.profit)}</b></span><span><small>必要利益</small><b>{yen(item.requiredProfit)}</b></span><span><small>あと</small><b>{yen(item.profitGap)}</b></span><span><small>Confidence</small><b>{item.confidence.toFixed(2)} / {item.requiredConfidence.toFixed(2)}</b></span></article>)}</div>}</section>
