  ...
  <div className="research-layout">
   <div className="research-primary">
    ...
    <section className="signal-panel" aria-label="直近判定の利益スナップショット">
     ...
     {chartData.length?<div className="signal-plot"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{top:3,right:8,bottom:0,left:0}}><CartesianGrid vertical={false} stroke="#e4e9ed"/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{fontSize:11,fill:"#718096"}}/><YAxis width={40} domain={[dataMin=>Math.min(0,Number(dataMin)),dataMax=>Math.max(5000,Number(dataMax))]} axisLine={false} tickLine={false} tick={{fontSize:11,fill:"#718096"}} tickFormatter={value=>`¥${Number(value).toLocaleString("ja-JP")}`}/><Tooltip formatter={value=>yen(Number(value))}/><ReferenceLine y={5000} stroke="#7c899a" strokeDasharray="4 4"/><ReferenceLine y={0} stroke="#b7c1cc"/><Bar dataKey="profit" name="検出利益" fill="#b1dbb8" barSize={18}/><Line dataKey="average" name="平均利益" type="monotone" stroke="#0f3e17" strokeWidth={2} dot={{r:2,fill:"#0f3e17"}}/><Line dataKey="moving" name="移動平均" type="monotone" stroke="#718096" strokeWidth={1.5} strokeDasharray="4 4" dot={false}/></ComposedChart></ResponsiveContainer></div>:<EmptyState>判定が蓄積されると利益分布を表示します。</EmptyState>}
    </section>

    <section className="watchlist-panel" aria-label="機会ウォッチリスト">
     <div className="analytic-heading"><div><h2>機会ウォッチリスト（ランキング）</h2><p>利益機会の高い順に並べています。価格・判定理由・商品一致度を比較できます。</p></div><div className="watchlist-filters" aria-label="判定フィルター"><button className={filter==="all"?"selected":""} onClick={()=>setFilter("all")}>すべて <b>{watchlist.length}</b></button><button className={filter==="buy"?"selected":""} onClick={()=>setFilter("buy")}>BUY <b>{buyCount}</b></button><button className={filter==="skip"?"selected":""} onClick={()=>setFilter("skip")}>SKIP <b>{skipCount}</b></button></div></div>
