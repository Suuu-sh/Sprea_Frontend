...
export default function Evaluations(){
 ...
 {loading?<div className="table-message">評価状態を読み込んでいます</div>:!error&&<><Section title="評価スケジュール" description="案件ごとの評価期限と結果です。"><div className="evaluation-table"><div className="evaluation-head"><span>商品</span><span>期限</span><span>状態</span><span>結果</span><span>利益</span></div>{schedules.map((x,i)=><div className="evaluation-row" key={`${x.decisionId}-${x.horizonHours}-${i}`}><span><Link href={`/products/?key=${encodeURIComponent(x.canonicalKey)}`}>{x.title}</Link><small>{horizon(x.horizonHours)}</small></span><span>{date(x.dueAt)}</span><span><b className={`status-pill ${x.status==="completed"?"active":"ready"}`}>{x.status==="completed"?"評価済み":"待機中"}</b></span><span>{x.outcome||"—"}</span><strong className={(x.profit??0)>=0?"positive":"negative"}>{x.profit===undefined?"—":yen(x.profit)}</strong></div>)}{!schedules.length&&<EmptyState>評価スケジュールはまだありません。</EmptyState>}</div></Section>
