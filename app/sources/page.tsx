...
export default function Sources(){
 ...
 const queueStateTone=queueStatus?.state??"idle";
 const queueStateDetail=queueStateTone==="running"?"販売APIを順番に検索しています":queueStateTone==="rebuild_pending"?"CSV取込後のキュー更新を待っています":queueStateTone==="failed"?"前回の実行でエラーが発生しました":queueRun?"次の探索タイミングを待っています":"探索状態を確認中です";
 ...
 return <AppShell title="データ取得元" description={`${dataLabel}の取得元と実稼働状況を表示します。`} badge={environmentBadge} actions={<button className="tool-button" onClick={()=>void load()} disabled={loading}><RefreshCw/>更新</button>}>
  ...
  <Section title="販売API探索キュー" description="候補を少量ずつ販売APIへ送り、結果が確認できた商品だけを次の画面へ進めます。" actions={<span className={`queue-state-badge ${queueStateTone}`}><i aria-hidden="true"/>{queueStateLabel}</span>}>
   ...
   {queueProviders.length>0&&<div className="queue-provider-list"><div className="queue-provider-heading"><span>Provider別の直近状況</span><small>今回のバッチ</small></div>{queueProviders.map(provider=><div className="queue-provider-row" key={provider.provider}><div className="queue-provider-name"><span>{provider.provider.slice(0,1).toUpperCase()}</span><div><b>{providerLabel(provider.provider)}</b><small>{provider.searched.toLocaleString("ja-JP")}ペア処理 ・ 利益条件 {provider.threshold.toLocaleString("ja-JP")}件</small></div></div><div className="queue-provider-result">{provider.failures>0?<><TriangleAlert/>失敗 {provider.failures}件</>:<><CheckCircle2/>正常</>}</div></div>)}</div>}
