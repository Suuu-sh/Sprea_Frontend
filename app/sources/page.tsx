...
export default function Sources(){const[last,setLast]=useState<CollectorRun|null>(null);useEffect(()=>{getCollectorStatus().then(x=>setLast(x.lastRun))},[]);return <AppShell title="データ取得元" description="取得方法・利用可否・環境分離を一か所で管理します。" badge="SAFE MODE"><CollectorHealth run={last}/><SourceList sources={sources}/></AppShell>}
