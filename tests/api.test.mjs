...
test("dashboard uses the display API and accepts its payload", async () => {
  ...
  await withApi((req, res) => { assert.equal(req.url, "/api/research/dashboard"); res.setHeader("content-type", "application/json"); res.end(JSON.stringify(payload)); }, async api => assert.deepEqual(await api.getResearchDashboard(), payload));
});

test("paper trade close posts to the dedicated action endpoint", async () => {
  await withApi((req, res) => { assert.equal(req.method, "POST"); assert.equal(req.url, "/api/research/paper-trades/42/close"); res.setHeader("content-type", "application/json"); res.end(JSON.stringify({ id: 42, status: "closed" })); }, async api => assert.equal((await api.closePaperTrade(42)).status, "closed"));
});
