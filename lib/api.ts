...
export type DiscoveryFunnel = { buybackQuotes: number; candidates: number; canonicalProducts: number; yahooFound: number; purchasable: number; profitable: number; threshold: number; buys: number; providers: DiscoveryProviderMetric[]; lastProviderRuns: Array<{ provider: string; searched_count: number; failure_count: number }>; lastRun: Record<string, unknown> | null };
