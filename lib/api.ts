...
export type DiscoveryProviderMetric = { provider: string; found: number; listings: number; profitable: number; threshold: number; averageProfitGap: number };
export type DiscoveryFunnel = { minimumProfit?: number; buybackQuotes: number; candidates: number; canonicalProducts: number; yahooFound: number; purchasable: number; profitable: number; threshold: number; buys: number; providers: DiscoveryProviderMetric[]; lastProviderRuns: Array<{ provider: string; searched_count: number; failure_count: number }>; lastRun: Record<string, unknown> | null };
...
export type KaitorixCsvStatus = {
  configured: boolean;
  today: string;
  objectKey: string;
  archivedToday: boolean;
  latest: CollectorRun | null;
  progress?: {
...
export type DiscoveryQueueStatus = {
  ...
  providers: Array<{ provider: string; searched: number; found: number; listings: number; profitable: number; threshold: number; failures: number; lastError?: string }>;
};
