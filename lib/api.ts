...
export type ResearchDashboard = { portfolio: { initialCapital: number; lockedCapital: number; availableCash: number; openTrades: number }; opportunities: ResearchOpportunity[]; decisions: ResearchDecision[]; decisionObservability: DecisionObservability; metrics48h: { evaluated: number; buyCount: number; precision: number; recall: number; missedOpportunities: number; averageProfit: number; maximumLoss: number } };
